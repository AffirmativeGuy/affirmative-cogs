...
class cop(commands.Cog):
    ...
    async def bank_balance(self, ctx, user: discord.Member = commands.Author):
        ...
        if bal <= 1000:
         ...
         await ctx.send(msg)
            ...
            msg = f"You have {bal} {currency} in your Bank account.\n-# Bro escaped the Matrix <:nervous:1261967406595571753>!"
            await ctx.send(msg)
    @commands.command()
    ...
    async def uptime(self, ctx: commands.Context):
        """Show's cop's uptime"""
        delta = datetime.datetime.utcnow() - self.bot.uptime # Adapted from core command
        uptime = humanize_timedelta(timedelta=delta) or ("Less than one second.")
        await ctx.send(
            ("I have not slept since **{time_quantity} <a:sad:1260965471675617340>!**").format(
